...
from classifier.train_edge_classifier import classify_build_conv,  grab_batches_with_generator, classify_rnn_2d, classify_build_conv_6d,  prepare_data_multiple_directories
...
def main(rgb_image_feat, scores_df):
    ...
    save_classifier_weights_to = './temp_models_in_progress/global_classifier/edge.h5'
    load_classifier_weights_dir = './temp_models_in_progress/global_classifier/edge.h5'
    retrain_classifier = True

    save_score_model_weights_to = './temp_models_in_progress/global_score'
    load_score_model_weights_dir = './temp_models_in_progress/global_score'
    ...
    num_classes = 2
    nono_list = []
    global_feats = [feat for feat in os.listdir('D:/scoring_and_profiling/')]
    global_feats = [feat for feat in global_feats if not(('edges' in feat) or ('dct' in feat) or ('fft' in feat) or ('cwt' in feat) or ('waves' in feat) or ('aug' in feat) or ('Teeth' in feat) or ('Crest' in feat) or ('Crop' in feat))]
    global_feats = [feat for feat in global_feats if (feat!='TrsSeal') or (feat!='100ovi') or (feat!='Stripe')]
    ...
    def edge(load_weights_dir, img_folder_to_classify):
        ...
        x_train, x_test, x_val, y_train, y_test, y_val = prepare_data_multiple_directories(global_feats, seed=420)
