...
if __name__ == '__main__':
    for folder in os.listdir('D:/scoring_and_profiling/'):
        if ('edges' in folder) or ('dct' in folder) or ('fft' in folder):
            continue
        folder_to_score = 'D:/scoring_and_profiling/' + folder + '/'
        print('-------' + folder_to_score + '--------')
        scored_frame = pd.DataFrame(index=['_'.join(note.split('_')[0:2]) for note in os.listdir('D:/scoring_and_profiling/FedSeal/')])
        ...
        scored_frame.to_csv('./' + folder + '_cmd.csv')
