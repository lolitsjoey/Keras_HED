...
def main(rgb_image_feat, scores_df):
    ...
    tool_images_in_this_folder = 'D:/scoring_and_profiling/TrsSealCrest/'
    spit_tool_output_here = 'D:/scoring_and_profiling/TrsSealCrest_edges/'
    ...
    def edge(load_weights_dir, img_folder_to_classify):
        ...
        model = classify_build_conv_6d(load_weights_dir)
        return x_train_total, x_test_total, x_val_total, y_train, y_test, y_val, train_batches, test_batches, val_batches, model
...
def get_dense_output(dense_df, array_of_images, model, layer_name, imShape = (480,480), write = False, edge=False, dct=False, TEMP=False, lbp_class=None):
    ...
    truth = []
    index_order = []
    for idx, (img_0, img_1) in enumerate(zip(os.listdir(img_folder_to_classify[0]), os.listdir(img_folder_to_classify[1]))):
        if idx == 0:
            print(img_0)
        if TEMP:
            test_img = cv2.resize(cv2.imread(img_folder_to_classify[0] + img_0, 3), imShape)
            hist = lbp_class.describe(test_img)
            test_img = cv2.resize(cv2.imread(img_folder_to_classify[1] + img_1, 0), imShape)
