...
if __name__ == '__main__':
    with tf.device('CPU:0'):
        load_classifier_weights_dir = './model_dir/classifiers/im_so_silly_dctversion.h5.h5'
        save_classifier_weights_to = './model_dir/classifiers/im_so_silly_dctversion.h5'
        retrain_classifier = True

        save_score_model_weights_to = './GAM_model/models/im_so_silly_dctversion'
        load_score_model_weights_dir = './GAM_model/models/im_so_silly_dctversion'
        ...
        spit_tool_output_here = 'D:/FRNLib/stripes/autodct/'

        load_tool_weights_from = ['./model_dir/finetuneencoderweights.h5', './model_dir/finetunedecoderweights.h5']
        ...
        def dct(load_weights_dir, img_folder_to_classify):
            ...
            train_batches, test_batches, val_batches = grab_batches_with_generator(x_train, x_test, x_val,
                                                                                   y_train, y_test, y_val, batchSize, blur=False)
        ...
        x_train, x_test, x_val, y_train, y_test, y_val, train_batches, test_batches, val_batches, model = dct(load_classifier_weights_dir, img_folder_to_classify)
        ...
        dense_output = pd.DataFrame(columns=range(n_dense_neurons), index=range(len(os.listdir(img_folder_to_test_classifier))))
        dense_output = get_dense_output(dense_output, img_folder_to_test_classifier, model, layer_name, write=True, dct=True)
        ...
        scores_df['autodct_scores'] = ordered_scores[np.argsort(arguments)]
